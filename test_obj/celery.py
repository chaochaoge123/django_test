...
project_settings = '%s.settings.test' % project_name

# 设置环境变量
os.environ.setdefault('DJANGO_SETTINGS_MODULE', project_settings)

# 实例化Celery
app = Celery('test_obj_task')

# 使用django的settings文件配置celery
app.config_from_object(project_settings)
...
发布任务：celery -A test_obj beat --pool=solo -l info
发布和执行一起：  celery -B -A test_obj  worker  --pool=solo -l info
